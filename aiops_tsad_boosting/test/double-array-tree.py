...
class DoubleArrayTrie(object):
    __slots__ = ['check', 'used', 'base', 'size', 'allowSize', 'error', 'keys', 'values', 'lengths', 'nextCheckPos',
                 'keySize', 'progress'
                            'vaues', 'progress']

    def __init__(self):
        self.check = np.zeros(shape=[65536 * 64], dtype=np.int64)
        self.base = np.zeros(shape=[65536 * 64], dtype=np.int64)
        self.used = np.zeros(shape=[65536 * 64], dtype=np.bool)
        self.size = 0
        self.allowSize = 0
        self.error = 0
        self.keys = []
    ...
    def __resize(self, newsize):
        ...
        newbase = np.hstack([self.base, np.zeros(shape=[newsize - size], dtype=np.int64)])

        newcheck = np.hstack([self.check, np.zeros(shape=[newsize - size], dtype=np.int64)])
    ...
    class Word():
        ...
        def __str__(self):
            tostring = 'value : %s   name  :%s   star : %d   end : %d' % (
            str(self.value), self.name, self.star, self.end)

            return tostring


if __name__ == '__main__':
    word_dict = {'zhang san': {'张三', '张散' }, 'shi jian': '时间'}

    import datetime
    start = datetime.datetime.now()
    dat = DoubleArrayTrie()
    dat.build(word_dict)
    a = dat.Search_word('zhang san he li si zai tong yi shi jian')
    end = datetime.datetime.now()
    print(end-start)
    for k in a:
        print(k)

    # value : {'张散', '张三'}   name  :zhang san   star : 0   end : 9
    # value : 时间   name  :shi jian   star : 31   end : 39