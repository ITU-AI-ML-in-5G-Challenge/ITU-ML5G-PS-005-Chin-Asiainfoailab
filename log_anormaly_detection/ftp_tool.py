...
class FTPTool(object):
    ...
    def downloaddirs(self, remotepath, localpath) -> int:
        ...
        if len(nlst_files) == 1:
            # FTP 文件列表为1, 表示文件路径，或文件夹下文件数量为 1 
            _next_files = nlst_files[0]
            # 重定向本地路径
            # 避免路径中出现 \\ 不使用os.path.join
            # localpath = os.path.join(localpath, os.path.basename(remotepath))

            if _next_files == remotepath:
                # 路径为文件时， 下载该文件
                localpath = "%s/%s" % (localpath, os.path.basename(remotepath)) 
                self.__downloadfile(_next_files, localpath)
                return 1
            else:
                # 路径为目录时，迭代本方法
                self.downloaddirs(_next_files, localpath)
        else:
            for f in nlst_files:
                # 遍历 FTP 文件目录
                self.downloaddirs(f, localpath)
...
if __name__ == '__main__':
    ...
    ftp_tool.error()
    # 从FTP服务器下载文件， 到指定目录
    # ftp_tool.downloaddirs("/data/input/faultEarlyWarning/predict/85b0969eb11d43b9a54e4b5d677dd661/20210712164142590","/home/songy/weiqs_workspace/abnormal_warning/test_output")
    # 从本地上传指定文件到FTP目录
